...
@push('scripts')
                ...
                success: function(response) {
                    ...
                    if (response.label == 'Inspection Request') {
                        ...
                        $.each(response.customers, function(indexInArray, valueOfElement) {
                            var selected = '{{ $invoice->user_id }}' == indexInArray ? 'selected' : '';
                            options += '<option value="' + indexInArray + '" ' + selected + '>' + valueOfElement + '</option>';
                        });
    ...
    <script>
        ...
        $(document).on('change', '#customer', function() {
            if ($('#invoice_type').val() == 'mobileservice' || $('#invoice_type').val() == 'vehicleinspection' || $('#invoice_type').val() == 'machinerepair' ||
                $('#invoice_type').val() == 'cardealership' || $('#invoice_type').val() == 'product' || $('#invoice_type').val() == 'project' || $('#invoice_type').val() == 'case' || $('#invoice_type').val() == 'sales' || $('#invoice_type').val() == 'newspaper')
            {
                    ...
                    success: function(data) {
                        if ($('#invoice_type').val() == 'childcare' && data.status == 'success') {
                            ...
                            $.each(data, function(key, value) {
                                ...
                                $('#course_order').append('<option value="' + key + '" ' + (key == abc ? 'selected' : '') + '>' + value + '</option>');
                            });
                        }  else {
                            if (data != '') {
    ...
    <Script>
        $(document).on('keyup', '.quantity', function() {
            ...
            if($('#invoice_type').val() == 'vehicleinspection' || $('#invoice_type').val() == 'machinerepair' || $('#invoice_type').val() == 'mobileservice'){
        ...
        <script>
            ...
            function items(data) {
                ...
                if (in_type == 'product' || in_type == 'sales' || in_type == 'vehicleinspection' || in_type ==
                    'machinerepair' || in_type == 'cardealership' || in_type == 'musicinstitute' || in_type == 'mobileservice') {
    ...
    @if (
        (module_is_active('Taskly') && $invoice->invoice_module == 'taskly') ||
            (module_is_active('LMS') && $invoice->invoice_module == 'lms'))
        <script>
            $(document).on('change', '.item', function() {
